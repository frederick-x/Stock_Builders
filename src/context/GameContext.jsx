...
export function GameProvider({ children }) {
  // Local storage helpers with backward-compat to older NEXORA keys
  ...
  }, [cashBalance, gemBalance, unlockedAgentIds, activeAgentId, holdings, missions, tradeLog, totalRealizedProfit, stocks, customApiKey, llmEnabled]);

  // When LLM mode is enabled, optionally augment/override agent advice with an LLM-generated recommendation
  useEffect(() => {
    ...
    (async () => {
      try {
        const prompt = `You are a concise trading assistant. Given agent ${activeAgent.name} with stats ${JSON.stringify(activeAgent.stats)} and stock ${selectedStock.symbol} priced at ${selectedStock.price}, recommend an action (BUY or SELL), the number of shares (integer), and a short rationale. Reply with JSON like {"action":"BUY","shares":10,"rationale":"..."} or plain text.`;
        ...
        setAgentAdvice(prev => {
          if (!parsed) {
            return { ...prev, reason: String(text).slice(0, 1000) };
          }
          return {
            ...prev,
