...
export function NexoraProvider({ children }) {
  // 1. User & Auth (managed by Supabase)
  ...
  // Supabase auth helpers (secure flow)
  const signInWithEmail = async (email, password) => {
    ...
    if (isSupabaseConfigured && supabase) {
      ...
      if (sessUser) {
        setUser({ id: sessUser.id, email: sessUser.email, name: sessUser.user_metadata?.full_name || sessUser.user_metadata?.name || sessUser.email.split('@')[0] });
        setIsAuthModalOpen(false);
        showToast(`Welcome back, ${sessUser.user_metadata?.full_name || sessUser.email.split('@')[0]}!`, 'success');
      }
      return { data, error };
    }

    // Local fallback: validate against local storage users and require confirmation
    try {
      const dbRaw = localStorage.getItem('LOCAL_USERS');
      const db = dbRaw ? JSON.parse(dbRaw) : [];
      const acct = db.find(u => u.email === email);
      if (!acct) return { error: new Error('No account found for that email') };
      if (!acct.confirmed) return { error: new Error('Please confirm your email before signing in') };
      if (acct.password !== password) return { error: new Error('Invalid credentials') };
      const localUser = { id: acct.id, email: acct.email, name: acct.name };
      setUser({ ...localUser, isLoggedIn: true });
      setIsAuthModalOpen(false);
      showToast(`Signed in locally as ${acct.name}`, 'success');
  ...
  const signUpWithEmail = async (email, password, fullName) => {
    // Supabase path: create account but do not auto-login; require email confirmation
    if (isSupabaseConfigured && supabase) {
      const { data, error } = await supabase.auth.signUp({ email, password, options: { data: { full_name: fullName } } });
      if (error) return { error };
      // Do NOT auto-login on sign-up. Require explicit sign-in to enter the game.
      showToast('Account created. Check your email to confirm before signing in.', 'info');
      return { data, error };
    }

    // Local fallback: store account with confirmed=false and DO NOT auto-login
    try {
      const dbRaw = localStorage.getItem('LOCAL_USERS');
      const db = dbRaw ? JSON.parse(dbRaw) : [];
      if (db.some(u => u.email === email)) return { error: new Error('An account with that email already exists') };
      const newAcct = { id: `local-${Date.now()}`, email, password, name: fullName || email.split('@')[0], confirmed: false };
      db.push(newAcct);
      localStorage.setItem('LOCAL_USERS', JSON.stringify(db));
      showToast('Local account created. Confirm email (simulate confirmation) before signing in.', 'info');
      return { data: { user: { id: newAcct.id, email: newAcct.email, name: newAcct.name } }, error: null };
    } catch (e) {
      return { error: e };
    }
  };

  const simulateConfirmLocalUser = (email) => {
    try {
      const dbRaw = localStorage.getItem('LOCAL_USERS');
      const db = dbRaw ? JSON.parse(dbRaw) : [];
      const idx = db.findIndex(u => u.email === email);
      if (idx === -1) return { error: new Error('Account not found') };
      db[idx].confirmed = true;
      localStorage.setItem('LOCAL_USERS', JSON.stringify(db));
      showToast(`Email for ${email} confirmed (local simulation). You may now sign in.`, 'success');
      return { ok: true };
    } catch (e) {
      return { error: e };
    }
  ...
  const handleGuest = () => {
    setUser({ name: 'Guest Explorer', email: `guest+${Date.now()}@stockbuilders.ai`, isLoggedIn: true, avatar: 'guest' });
    setIsAuthModalOpen(false);
    showToast('Logged in as Guest Explorer (Simulation Active)', 'info');
  };

  // Sync supabase session on mount and subscribe to auth changes
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
        ...
        if (session && mounted) {
          ...
          setUser({ id: u.id, email: u.email, name: u.user_metadata?.full_name || u.user_metadata?.name || u.email.split('@')[0] });
          setIsAuthModalOpen(false);
        } else if (mounted) {
          setUser(null);
          setIsAuthModalOpen(true);
        ...
        const { data: sub } = supabase.auth.onAuthStateChange((event, sessionData) => {
